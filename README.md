...
- **Custom `pi_sd` only:** ensure something **writes 32-byte frames** to the FIFO (or stdin). The server opens FIFOs in **read+write** mode so startup does **not** block forever waiting for an external writer (Linux `fifo(7)` pattern); you still need a process that **writes** framed bytes for real audio. All-zero dBFS with rising **blocks** usually means the stream is zeros or not connected.
